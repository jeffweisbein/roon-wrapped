...
router.get("/now-playing", async (req, res) => {
  try {
    ...
    const recommendations = await recommendationEngine.getTrackRecommendations(
      track,
      8,
    );
...
router.get("/personalized", async (req, res) => {
  try {
    ...
    const excludeArtists = req.query.exclude
...
router.get("/mood/:mood", async (req, res) => {
  try {
    ...
    const recommendations = await recommendationEngine.getMoodRecommendations(
      mood,
      limit,
    );
...
router.get("/similar-artists/:artist", async (req, res) => {
  try {
    ...
    const similar = await musicMetadata.getSimilarArtists(
...
router.post("/feedback", async (req, res) => {
  try {
    ...
    if (!recommendation || !action) {
      return res
        .status(400)
        .json({ error: "Missing recommendation or action" });
    }
...
router.get("/preferences", async (req, res) => {
  try {
    const preferenceManager = require("../services/preference-manager");
    const profile = preferenceManager.getPreferenceProfile();
    const favoriteArtists = preferenceManager.getFavoriteArtists();
    const cooldownArtists = preferenceManager.getArtistsOnCooldown();

    res.json({
      profile,
      favoriteArtists,
      cooldownArtists,
      discoveryStats: recommendationEngine.getDiscoveryStats(),
    });
  } catch (error) {
...
router.post("/replacement", async (req, res) => {
  try {
    const { excludeArtists = [] } = req.body;

    const replacement = await recommendationEngine.getReplacementRecommendation(
      excludeArtists,
    );

    res.json({ recommendation: replacement });
  } catch (error) {
    logger.error("Error getting replacement recommendation:", error);
    res.status(500).json({ error: "Failed to get replacement" });
  }
});
