...
class RecommendationEngine {
  ...
  async analyzeUserProfile() {
    ...
    const peakHour = timeOfDayListening.indexOf(Math.max(...timeOfDayListening));
  ...
  async getPersonalizedRecommendations(limit = 20) {
    ...
    const familiarArtists = new Set(this.userProfile.topArtists.map(a => a.artist.toLowerCase()));
    const allListenedArtists = new Set(this.listeningHistory.map(t => t.artist?.toLowerCase()).filter(Boolean));
    ...
    for (const [artist, playCount] of artistPlayCounts) {
      ...
      if ((playCount >= 1 && playCount <= 10) || (uniqueSongs >= 1 && uniqueSongs <= 3)) {
    ...
    const genreExpansion = {
      'pop punk': ['indie rock', 'post-hardcore', 'ska punk', 'synthpop', 'indie pop'],
    ...
    for (const { artist, playCount, uniqueSongs } of lightlyListenedArtists.slice(0, 3)) { // Reduced from 5
      try {
        const artistInfo = await musicMetadata.getArtistInfo(artist);
        if (artistInfo) {
          recommendations.set(`explore_${artist}`, {
            ...
            reasons: [`You've only heard ${uniqueSongs} song${uniqueSongs > 1 ? 's' : ''} - explore more!`],
            isFamiliar: false,
            type: 'deep_dive',
    ...
    for (const { artist } of topArtistsToCheck) {
      try {
        ...
        for (const simArtist of similar) {
          if (simArtist.match >= 0.3) { // Lower threshold to get more diverse results
            const isFamiliar = allListenedArtists.has(simArtist.name.toLowerCase());
            ...
            if (simArtist.match >= 0.5 && recommendations.size < limit) {
              try {
                ...
                logger.warn(`Failed to get info for similar artist ${simArtist.name}`);
              }
            }
          ...
          const score = simArtist.match * (isCrossGenre ? 1.5 : 1) * (simArtist.match < 0.6 ? 1.2 : 0.8);
  ...
  async trackFeedback(recommendation, action) {
    ...
    // This would update the discovery ratio and similarity thresholds
    
    if (action === 'played' || action === 'liked') {
      if (!recommendation.isFamiliar && this.discoveryRatio < 0.5) {
        this.discoveryRatio += 0.02; // Slowly increase discovery
      }
    } else if (action === 'skipped' || action === 'disliked') {
      if (!recommendation.isFamiliar && this.discoveryRatio > 0.2) {
        this.discoveryRatio -= 0.02; // Reduce discovery
      }
    }

    logger.info(`Feedback tracked: ${action} for ${recommendation.artist}`, {
