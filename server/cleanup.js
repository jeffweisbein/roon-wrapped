...
const HISTORY_FILE = path.join(__dirname, '..', 'data', 'listening-history.json');
...
function cleanupDuplicates() {
    ...
    const recentTracks = history.filter(track => new Date(track.timestamp).getTime() > oneHourAgo);
    const olderTracks = history.filter(track => new Date(track.timestamp).getTime() <= oneHourAgo);
    ...
    recentTracks.forEach(track => {
        ...
        if (!existingTrack) {
            ...
            const timeDiff = new Date(track.timestamp).getTime() - new Date(existingTrack.timestamp).getTime();
            const seekDiff = Math.abs(track.seek_position - existingTrack.seek_position);
    ...
    // Combine and save
    const newHistory = [...olderTracks, ...cleanedRecentTracks].sort((a, b) => 
    ...
    console.log(`Removed ${recentTracks.length - cleanedRecentTracks.length} duplicates`);
