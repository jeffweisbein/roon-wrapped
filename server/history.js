...
router.get("/status", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get status", lastError: err.message });
  }
});
...
router.get("/now-playing", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Failed to get now playing info",
...
router.get("/image/:key", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get image", lastError: err.message });
  }
});
...
router.post("/restart", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to restart", message: error.message });
  }
});
...
router.get("/", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get history", message: error.message });
  }
});
...
router.get("/wrapped/:year?", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get wrapped data", message: error.message });
  }
});
...
router.post("/refresh-zones", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to refresh zones", message: error.message });
  }
});
