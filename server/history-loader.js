const loadHistoryData = async (browse) => {
    ...
    try {
        ...
        const historyItem = root.items.find(item => 
            item.title === "History" || 
            item.title === "Play History"
        );
        ...
        while (hasMore) {
            ...
            console.log(`Processing ${currentList.items.length} items from ${currentList.title || 'list'}`);
                    ...
                    try {
                        if (item.input_prompt) {
                            ...
                            if (currentList.title.includes('Today')) {
                                ...
                                timestamp = new Date().setHours(0, 0, 0, 0) - (24 * 60 * 60 * 1000);
                            } else {
        ...
        console.log(`History data loaded successfully. Total tracks: ${loadedItems}`);
        await historyService.saveHistory();
