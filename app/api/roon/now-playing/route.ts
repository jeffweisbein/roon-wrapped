...
export async function GET() {
  try {
    const response = await fetch(
      `http://${ROON_SERVER_HOST}:${ROON_SERVER_PORT}/api/roon/now-playing`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch now playing: ${response.status} ${response.statusText}`,
      );
    }
    ...
    const nowPlayingData = serverResponse.success
      ? serverResponse.data
      : serverResponse;
