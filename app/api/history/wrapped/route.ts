...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[Frontend API] Backend error: ${response.status} ${response.statusText}`,
      );
    ...
    if (!data.totalTracksPlayed && data.totalTracksPlayed !== 0) {
      console.warn(
        "[Frontend API] Missing totalTracksPlayed in backend response",
      );
    }
    ...
    const wrappedData = {
      ...
      topArtistsByPlays: Array.isArray(data.topArtistsByPlays)
        ? data.topArtistsByPlays
        : [],
      topAlbumsByPlays: Array.isArray(data.topAlbumsByPlays)
        ? data.topAlbumsByPlays
        : [],
      topTracksByPlays: Array.isArray(data.topTracksByPlays)
        ? data.topTracksByPlays
        : [],
      topGenresByPlays: Array.isArray(data.topGenresByPlays)
        ? data.topGenresByPlays
        : [],
      listeningPatterns: {
